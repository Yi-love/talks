$(document).ready(function(){
	...
	var heartindex = 0;
	//切换心里话类型
	$('.select_bar').on('click' , function(evt){
		var target = evt.target || window.event.srcElement;
		$('.select_bar div').removeClass('select_choice');
		$(target).addClass('select_choice');
		$('.say-content').html('');
		heartindex = 0;
		if ( $(target).hasClass('heartall') ){
			place = 0;
			findHeartByPlace(place);
		}else if ( $(target).hasClass('friendheart') ){
			place = 1;
			findHeartByPlace(place);
		}else if ( $(target).hasClass('userheart') ){
			place = 2;
			findHeartByPlace(place);
		}
	});
	...
	function findHeartByPlace(place){//查找心里话
		$.get('/userindex/findheartplace/'+heartindex+'/'+place , function(data){
			// console.log(data);
			var heartlist = data.heartlist;
			var temp = '';
			if ( heartlist.length > 0 ){
				for ( var i = 0  , len = heartlist.length ; i < len ; i++ ){
					temp += '<li class="say-item"><div class="say-item-content"><div class="say-user"><div class="user-img-box"><div class="user-img"><img src="'+heartlist[i].userimg+'" alt=""></div></div><div class="say-title"><h3><a href="/user/'+heartlist[i].userid+'">'+heartlist[i].nickname+'</a></h3><h6>'+heartlist[i].writetime+'</h6></div><span class="pushpin"><i class="icon-star-empty"></i></span></div><a href="/userindex/heartinfo/'+heartlist[i]._id+'"><p class="article-content">'+heartlist[i].content+'</p></a><div class="article-img-box">';
...
// 转发    点赞    收藏
	$('.say-content').on('click' , function(evt){
		var target = evt.target || window.event.srcElement;
		if ( $(target).hasClass('transmit') && target.nodeName == 'I' ){//转发
		...
		}else if ( $(target).hasClass('agree') && target.nodeName == 'I' ){//点赞
			var heartid = $(target).attr('data-id');
			$.ajax({
				type: "POST",
				url: "/userindex/agree",
		...
		}else if ( $(target).hasClass('collection') && target.nodeName == 'I' ){//收藏
			var heartid = $(target).attr('data-id');
			$.ajax({
				type: "POST",
